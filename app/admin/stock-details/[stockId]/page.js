...
const StockDetails = ({ params }) => {
  ...
  const { data: stock } = useAsync(getStock, [stockId], [stockId]);
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <div className="space-y-1 rounded p-3 shadow">
              ...
              <p className="text-gray-800">{reshapePrice(stock?.data?.purchasePrice)}</p>
            </div>
            <div className="space-y-1 rounded p-3 shadow">
              <h2 className="text-md font-semibold">Selling Price</h2>
              <p className="text-gray-800">{reshapePrice(stock?.data?.sellingPrice)}</p>
            </div>
            <div className="space-y-1 rounded p-3 shadow">
              <h2 className="text-md font-semibold">Total Price</h2>
              <p className="text-gray-800">{reshapePrice(stock?.data?.totalPrice)}</p>
            </div>
          </div>
