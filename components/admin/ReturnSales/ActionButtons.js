...
import { DeleteReturnSale } from "@/actions/storeAdmin/returnSale/returnSaleActions";
...
const ActionButtons = ({ id }) => {
  ...
  const handleDelete = async (salesId) => {
    ...
    try {
      ...
      if (isSure) {
        const result = await DeleteReturnSale(salesId);
        if (result?.data) {
          toast.success("Return Sales deleted successfully");
          router.refresh("/admin/return-sales");
        } else {
  ...
  return (
    <span className="flex w-full items-center justify-center gap-2">
      <Link href={`/admin/return-sales-details/${id}`}>
        <span>
          <svg
      ...
      <Link href={`/admin/return-sales-update/${id}`}>
        <span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
