...
const Sidebar = () => {
  ...
  return (
    <div>
      ...
      >
        <div className="sidebar-scrollbar-customize h-full overflow-y-auto bg-bg py-2 shadow-sm shadow-gray-400 backdrop-blur">
          <ul className="select-none space-y-1 text-[15px] font-medium">
            {sidebarRoutes(user?.permission || {})?.map((item, i) => (
              <MenuItems
                key={i}
                item={item}
                index={i}
