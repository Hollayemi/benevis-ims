...
const ActionButtons = ({ id }) => {
  ...
  return (
    <span className="flex w-full items-center justify-center gap-2">
      ...
      <Link href={`/admin/employee-update/${id}`} className={`${user?.permission === "accountant" && "hidden"}`}>
        <span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
      ...
      <button onClick={() => handleDelete(id)} disabled={loading} className={`${user?.permission === "accountant" && "hidden"}`}>
        <span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
